...
import { exportAuditLogToPDF, exportAuditLogToCSV, exportAuditLogToJSON } from '../../../../utils/auditLogExport';
...
const AuditTrail = ({ caseId, caseData }) => {
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const result = await exportAuditLogToPDF(filteredLogs, caseData);
      
      if (result.success) {
        setToast({
          type: 'success',
          message: `Audit log exported to PDF: ${result.filename}`
        });
      } else {
        setToast({
          type: 'error',
      ...
      console.error('Error exporting audit log PDF:', error);
      setToast({
        type: 'error',
  ...
  const handleExportCSV = async () => {
    try {
      ...
      const result = exportAuditLogToCSV(filteredLogs, caseData);
      
      if (result.success) {
        setToast({
          type: 'success',
          message: `Audit log exported to CSV: ${result.filename}`
        });
      } else {
        setToast({
          type: 'error',
          message: `Failed to export CSV: ${result.error}`
        });
      }
    } catch (error) {
      ...
      setToast({
        type: 'error',
        message: 'An unexpected error occurred while exporting CSV'
      });
    } finally {
      setIsExporting(false);
    }
  ...
  const handleExportJSON = async () => {
    try {
      ...
      const result = exportAuditLogToJSON(filteredLogs, caseData);
      
      if (result.success) {
        setToast({
          type: 'success',
          message: `Audit log exported to JSON: ${result.filename}`
        });
      } else {
        setToast({
          type: 'error',
          message: `Failed to export JSON: ${result.error}`
        });
      }
    } catch (error) {
      console.error('Error exporting audit log JSON:', error);
      setToast({
        type: 'error',
        message: 'An unexpected error occurred while exporting JSON'
      });
    } finally {
      setIsExporting(false);
    }
      ...
      <div className="flex justify-center pt-6">
        <div className="relative">
          <button 
            ...
            className="inline-flex items-center px-6 py-3 bg-slate-800/50 hover:bg-slate-700/50 border border-slate-600 rounded-lg text-white transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          {showExportMenu && (
            <div className="absolute bottom-full mb-2 left-0 w-48 bg-slate-800/95 border border-slate-700/50 rounded-xl shadow-2xl backdrop-blur-lg z-50">
              <div className="py-2">
                <button
                ...
                <button
                  ...
                  className="flex items-center w-full px-4 py-2 text-sm text-slate-300 hover:bg-slate-700/50 hover:text-white transition-colors"
                >
                  <FaFileCode className="mr-3 text-blue-400" />
                  Export as JSON
                </button>
              </div>
            </div>
